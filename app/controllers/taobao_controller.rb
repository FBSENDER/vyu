...
class TaobaoController < ApplicationController
  def baobei
    ...
    @description = "【剁手都要买的宝贝(#{@product.title})】#{@product.title} - 分类#{@product.cate_name}/#{@product.cate_leaf_name}，原价#{@product.origin_price}元，现价#{@product.price}元，天猫淘宝#{@product.nick}热卖中，本月销量#{@product.volume}，优惠券领取，降价折扣查询，宝贝精选 - vxixi.com"
    @path = request.path + "/"
  ...
  def youxuan
    ...
    @title = "#{@keyword}_#{@keyword}【价格 图片 打折 包邮】_天猫淘宝#{@keyword}"
    @keywords = "#{@keyword},#{@keyword}价格,#{@keyword}图片,#{@keyword}打折,#{@keyword}包邮,淘宝#{@keyword},天猫#{@keyword}"
    @description = "#{@keyword}优惠专场，实时更新高性价比#{@keyword}单品、淘宝#{@keyword}打折特卖信息，全场低至1折起包邮，敬请关注！这些#{@keyword}都是由专业编辑为您精挑细选的，欢迎进入页面了解#{@keyword}价格、#{@keyword}图片等相关信息！vxixi.com致力于帮您节省浏览海量商品信息时间，让您以更优惠的折扣价格购买到自己喜欢的#{@keyword}！"
    @path = request.path + "/"
